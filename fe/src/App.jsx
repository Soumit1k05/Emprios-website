...
const BundleCard = ({ name, price, features, popular }) => (
  ...
  >
    {popular && (
      <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-blue-500 text-white text-[8px] font-black px-3 py-1 rounded-full uppercase tracking-widest">
        Most Popular
      </div>
    )}
...
export default function App() {
  ...
  const [view, setView] = useState('home'); // home | bundles | prototype | auth | account
  ...
  // If user logs out from AccountPage, go back to home
  const handleLogout = () => setView('home');

  // When auth succeeds, go to account
  const handleAuthSuccess = () => setView('account');
        ...
        <header className="h-[75px] px-8 flex items-center justify-between border-b border-white/10 shrink-0">
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-black uppercase tracking-[0.3em]">EMPIROS</span>
            </div>
          </div>
          ...
          <nav className="hidden md:flex items-center gap-10">
            ...
            <NavItem active={view === 'auth' || view === 'account'} onClick={handleAccountClick}>
              {user ? (
                <span className="flex items-center gap-1.5">
                  <div className="w-5 h-5 rounded-full bg-blue-600 flex items-center justify-center text-white text-[8px] font-black">
          ...
          <div className="flex items-center gap-2">
            {/* Mobile account icon */}
            <button onClick={handleAccountClick} className="md:hidden w-9 h-9 rounded-full bg-white/10 flex items-center justify-center">
              <User size={16} />
            </button>
            <div
              className="w-14 h-7 rounded-full bg-white/10 border border-white/20 p-1 flex items-center cursor-pointer relative"
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'home' && (
              <motion.div key="home" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-12">
                <div className="rounded-[45px] glass-pod text-center p-12 lg:p-20 max-w-2xl mx-auto">
                  <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="inline-block px-4 py-1 rounded-full bg-blue-500/10 text-red-500 text-[9px] font-black uppercase tracking-[0.2em] mb-6">
                    Introducing Empiros
                  </motion.div>
                  ...
                  <div className="flex flex-wrap justify-center gap-4">
                    <button onClick={() => setView('prototype')} className="px-10 py-5 bg-green-600 text-white rounded-3xl text-[10px] font-black uppercase tracking-widest shadow-2xl shadow-blue-500/40 hover:scale-105 transition-transform">
                      Get Started
                    </button>
                    <button onClick={() => setView('bundles')} className="px-10 py-5 bg-white/10 border border-white/5 rounded-3xl text-[10px] font-black uppercase tracking-widest backdrop-blur-md hover:bg-white/20 transition-all">
                      Select Bundle
                    </button>
                  </div>
                </div>
              ...
              <motion.div key="prototype" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
                <div className="glass-pod p-10 h-[500px] flex items-center justify-center relative overflow-hidden group">
                  ...
                  <div className="text-center space-y-4 relative z-10">
                    ...
                    <button onClick={() => setView('home')} className="text-[10px] font-black uppercase tracking-widest text-blue-500 border-b-2 border-blue-500 pb-1">
                      Exit Prototype
                    </button>
                  </div>
                </div>
            ...
            {view === 'auth' && (
              <AuthPage key="auth" onSuccess={handleAuthSuccess} />
            )}

            {view === 'account' && user && (
              <AccountPage key="account" onLogout={handleLogout} />
            )}
